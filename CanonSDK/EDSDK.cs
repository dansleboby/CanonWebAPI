...
namespace EDSDKLib
{
    ...
    public class EDSDK
    {
        ...
        public static uint EdsGetPropertyData(IntPtr inRef, uint inPropertyID, int inParam, out uint outPropertyData)
        {
            ...
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsTime));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsFocusInfo));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsPoint));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsRect));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsSize));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            EDSDK.EdsGetPropertySize(inRef, inPropertyID, 0, out var type, out var size);
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
            var len = size / 4;
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.EdsCameraPos));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var size = Marshal.SizeOf(typeof(EDSDK.FocusShiftSetting));
            var ptr = Marshal.AllocHGlobal(size);
            var err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);
        ...
        {
            var userdata = (EdsManualWBData)Marshal.PtrToStructure(ptr, typeof(EdsManualWBData));
            var headerSize = 40;
            var tmp = new byte[userdata.dataSize + headerSize];
            userdata.data = new byte[userdata.dataSize];
            Marshal.Copy(ptr, tmp, 0, (int)userdata.dataSize + headerSize);
            for (var i = 0; i < userdata.dataSize; i++)
            {
        ...
        //
        //  Description:
        //      This function acquires an event from a camera. 
